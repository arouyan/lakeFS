...
layout: default
title: Hive
description: This section shows how you can start using lakeFS with Apache Hive, a distributed data warehouse system that enables analytics on a massive scale.
parent: Integrations
nav_order: 25
has_children: false
...
{: .no_toc }
The [Apache Hive ™](https://hive.apache.org/) data warehouse software facilitates reading, writing, and managing large datasets residing in distributed storage using SQL. The tructure can be projected onto data already in storage. A command line tool and JDBC driver are provided to connect users to Hive.
...
**Note**
In this example we set `fs.s3a.path.style.access` to true to remove the need of additional DNS records for [virtual hosting](https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html)
