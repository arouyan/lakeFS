...
## Copying data with external tools

To import existing data to lakeFS, you can copy it using [S3 CLI](../integrations/aws_cli.md#copy-from-a-local-path-to-lakefs) 
or tools like [Apache DistCp](../integrations/distcp.md#from-s3-to-lakefs). This is the most straightforward way, and we recommend it if it’s applicable to your case.

## Limitations

Unfortunately, copying data is not always feasible for the following reasons:
1. Some data may be too large to copy.
...
To solve this, we provide an import tool that doesn't copy any data, allowing for a more gradual onboarding process.
...
It is important to note that due to the deduplication feature of lakeFS, the data will stay in your original bucket even
...
The import is performed by `lakefs` 's `import` command.
...
You will see the progress of your import as it's carried out.
...
As previously mentioned, the command listed above imports data to the dedicated `import-from-inventory` branch.
...
{: .no_toc }
1. Perform the import from a machine with access to your database and in the same region of your destination bucket.

1. You can download the `lakefs` binary from [here](https://github.com/treeverse/lakeFS/releases). Make sure you choose one compatible with your lakeFS Installation.
